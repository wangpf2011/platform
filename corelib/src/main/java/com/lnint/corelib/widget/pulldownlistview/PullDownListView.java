package com.lnint.corelib.widget.pulldownlistview;
...
public class PullDownListView extends FrameLayout implements
	...
	private void addRefreshBar() {
		...
		//向上滑动的动画
		mAnimationUp = AnimationUtils.loadAnimation(getContext(),R.anim.listview_rotate_up);
		mAnimationUp.setAnimationListener(this);
		
		//向下滑动的动画
		mAnimationDown = AnimationUtils.loadAnimation(getContext(),R.anim.listview_rotate_down);
		mAnimationDown.setAnimationListener(this);
		
		//刷新头部的view
		view = LayoutInflater.from(getContext()).inflate(R.layout.listview_refresh_bar,null);
		//添加view在本控件中
		addView(view);
		...
		mArrow.setImageResource(R.mipmap.listview_arrow_down);
		...
		int i = getResources().getDimensionPixelSize(R.dimen.listview_updatebar_padding);
	...
	protected void onFinishInflate() {
		...
		footer_layout = (LinearLayout) LayoutInflater.from(getContext())
				.inflate(R.layout.listview_empty_main, null);
		foot = (LinearLayout) LayoutInflater.from(getContext()).inflate(
				R.layout.listview_ref2, null);
		...
		if (useempty) {
			header = (LinearLayout) LayoutInflater.from(getContext()).inflate(
					R.layout.listview_empty_main, null);
	...
	private void init() {
		MAX_LENGHT = getResources().getDimensionPixelSize(
				R.dimen.listview_updatebar_height);// 62.0dip
		...
		mPading = -MAX_LENGHT;
		mLastTop = -MAX_LENGHT;
		pulldowntorefresh = getContext().getText(R.string.listview_drop_dowm).toString();
		releasetorefresh = getContext().getText(R.string.listview_release_update)
				.toString();
		loading = getContext().getText(R.string.listview_loading).toString();
	}
	...
	public void onFirstLoad() {
		...
		more.setText(R.string.listview_loading);
	}
	...
	public void onLoadMoreComplete(String date) {
		mState = -1;
		mProgressBar2.setVisibility(View.INVISIBLE);
		more.setText(R.string.listview_see_more);
		updateCommon();
		if (isFirstLoading)
	...
	public void onAnimationEnd(Animation animation) {
		...
		if (top < 0)
			...
			mArrow.setImageResource(R.mipmap.listview_arrow_up);
		else {
			if (top < mLastTop) {
				mArrow.setImageResource(R.mipmap.listview_arrow_down);
			} else {
