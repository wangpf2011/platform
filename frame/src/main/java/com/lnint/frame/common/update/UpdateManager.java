package com.lnint.frame.common.update;
...
public class UpdateManager {
	...
	public void checkAppUpdate(Context context, final boolean isShowMsg) {		
		...
		if(isShowMsg) {
			if(mProDialog == null) {
				mProDialog = CustomProgressDialog.createDialog(mContext);
				mProDialog.setMessage("正在检测，请稍后...");
				mProDialog.show();
		...
		http.send(HttpRequest.HttpMethod.GET,
				AppHelper.HTTPRUL + "userfiles/app/AppVersion.xml",
				null,
				new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						String resText = responseInfo.result;
						Map<String, String> result = XmlUtils.xmlElements(resText);

						Update update = new Update();
						if (result.get("versionCode") != null) {
							update.setVersionCode(Integer.valueOf(result.get("versionCode")));
						} else {
							update.setVersionCode(0);
						}
						update.setDownloadUrl(AppHelper.HTTPRUL + result.get("downloadUrl")+"?appName=workneed");
						update.setVersionName(result.get("versionName"));
	...
	private Runnable mdownApkRunnable = new Runnable() {	
		...
		public void run() {
			try {
				String apkName = "workneed.apk";
				String tmpApk = "workneed.tmp";
				//判断是否挂载了SD卡
				String storageState = Environment.getExternalStorageState();
				if(storageState.equals(Environment.MEDIA_MOUNTED)) {
					savePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/update/";
